...
public class DatasetGraphContractTest<T extends DatasetGraph> {
	...
	// Quad operations
	/*
	 * void add(Quad quad) void delete(Quad quad) void deleteAny(Node g, Node s,
	 * Node p, Node o) Iterator<Quad> find(Quad quad) Iterator<Quad> find(Node
	 * g, Node s, Node p , Node o) boolean contains(Node g, Node s, Node p ,
	 * Node o) boolean contains(Quad quad)
	 */
	@ContractTest
	public void quad_01() {
		DatasetGraph dsg = producer.newInstance();
	...
	/*
	 * getDefaultGraph() getGraph(Node) containsGraph(Node) ????
	 * setDefaultGraph(Graph) addGraph(Node, Graph) removeGraph(Node)
	 * listGraphNodes()
	 */
